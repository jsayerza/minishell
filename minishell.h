...
void	add_redirect_file_append(t_collector **collector,
			t_const *node, char *file);
void	add_heredoc(t_collector **collector, t_const *node, char *file);
void	set_pipe_flags_and_link(t_const *left, t_const *right);

/* constructor/constructor_add_redirs.c */
void	add_redirections(t_collector **collector, t_ast *ast, t_const *cmd);
...
char	**copy_env_excluding_var(char **env, const char *var_name, int env_len);
