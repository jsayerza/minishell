...
t_constructor *create_single_constructor_node(int node_number)
{
    ...
    // Si el tipo de token es TOKEN_BUILTIN, preguntar el tipo de builtin
    ...
    // Solicitar el número de ejecutables
    ...
    // Asignar memoria para los ejecutables
    ...
    // Solicitar los ejecutables
    for (int i = 0; i < executable_count; i++) {
        char executable[1024];
        printf("Ingrese el ejecutable %d: ", i + 1);
        if (!fgets(executable, sizeof(executable), stdin)) { // Corregido aquí
